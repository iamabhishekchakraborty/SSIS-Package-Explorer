...
{
    //Bulk Insert Task
    public class BulkInsertTaskSearcher:ISSISItem
    {
        string currentItemType = "Bulk Insert Task";
        ...
        public List<PackageItem> Search(string searchPattern, object executable, PackageInfo packageData, string parentName = "", string parentType = "")
        {
            ...
            BulkInsertTask bulkInsertTask = (BulkInsertTask)hostedTask.InnerObject;
            ...
            SSISItems.AddRange(SearchPattern(searchPattern, bulkInsertTask, hostedTask, packageData));

            return SSISItems;
        }

        private List<PackageItem> SearchPattern(string searchPattern, BulkInsertTask task, TaskHost hostedTask, PackageInfo packageData)
        {
            List<PackageItem> SSISItems = new List<PackageItem>();

            if (CommonUtility.CheckStringExistence(task.DestinationTableName, searchPattern))
            {
                PackageItem executeSQLTaskItem = new PackageItem();
                executeSQLTaskItem.Type = currentItemType + " -> DestinationTableName";
                executeSQLTaskItem.Name = currentItemName;
                executeSQLTaskItem.GUID = hostedTask.ID;
                executeSQLTaskItem.ConnectionName = task.DestinationConnection;
