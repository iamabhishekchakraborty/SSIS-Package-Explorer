...
namespace SSIS_Package_Reader.SSIS_Item_Factory
{
    ...
    public class DbMaintenanceNotifyOperatorTaskSearcher:ISSISItem
    {
        string currentItemType = "Notify Operator Task";
        ...
        public List<PackageItem> Search(string searchPattern, object executable, PackageInfo packageData, string parentName = "", string parentType = "")
        {
            ...
            DbMaintenanceNotifyOperatorTask DbMaintenanceNotifyOperatorTask = (DbMaintenanceNotifyOperatorTask)hostedTask.InnerObject;
            ...
            SSISItems.AddRange(SearchPattern(searchPattern, DbMaintenanceNotifyOperatorTask, hostedTask, packageData));

            return SSISItems;
        }

        private List<PackageItem> SearchPattern(string searchPattern, DbMaintenanceNotifyOperatorTask task, TaskHost hostedTask, PackageInfo packageData)
        {
            List<PackageItem> SSISItems = new List<PackageItem>();
            ...
            if (CommonUtility.CheckStringExistence(task.Message, searchPattern))
            {
                PackageItem taskNameItem = new PackageItem();
                taskNameItem.Type = currentItemType + " -> Message";
                taskNameItem.Name = currentItemName;
                taskNameItem.GUID = hostedTask.ID;
                taskNameItem.InterfaceName = packageData.Name;
                taskNameItem.FileName = packageData.FileName;
            ...
            if (CommonUtility.CheckStringExistence(task.Description[1].ToString().Replace("Operators to notify: ", ""), searchPattern))
            {
                PackageItem taskNameItem = new PackageItem();
                taskNameItem.Type = currentItemType + " -> Selected Operators to notify";
                taskNameItem.Name = currentItemName;
                taskNameItem.GUID = hostedTask.ID;
                taskNameItem.InterfaceName = packageData.Name;
                taskNameItem.FileName = packageData.FileName;
