...
{
    public class SQLServerCompactDestinationSearcher:ISSISItem
    {
        string currentItemType = "SQL Server Compact Destination";
        ...
        public List<PackageItem> Search(string searchPattern, object executable, PackageInfo packageData, string parentName = "", string parentType = "")
        {
            ...
            IDTSComponentMetaData100 SQLCEDest = (IDTSComponentMetaData100)executable;

            currentItemName = (parentName != "") ? parentName + " -> " + SQLCEDest.Name : SQLCEDest.Name;
            currentItemType = (parentType != "") ? parentType + " -> " + currentItemType : currentItemType;

            if (CommonUtility.CheckStringExistence(SQLCEDest.Name, searchPattern))
            {
                PackageItem taskNameItem = new PackageItem();
                taskNameItem.Type = currentItemType + " -> Name";
                taskNameItem.Name = currentItemName;
                taskNameItem.GUID = SQLCEDest.ComponentClassID;
        ...
        private List<PackageItem> SearchPattern(string searchPattern, IDTSComponentMetaData100 component, PackageInfo packageData)
        {
            ...
            try
            {
                var propertyValue = component.CustomPropertyCollection["Table Name"].Value;
                string tableName = propertyValue == null ? "" : propertyValue.ToString();
                if (CommonUtility.CheckStringExistence(tableName, searchPattern))
                {
                    PackageItem taskItem = new PackageItem();
                    taskItem.Type = currentItemType + " -> Table Name";
