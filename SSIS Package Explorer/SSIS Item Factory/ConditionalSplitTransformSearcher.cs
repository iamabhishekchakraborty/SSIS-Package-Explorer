...
namespace SSIS_Package_Reader.SSIS_Item_Factory
{
    ...
    public class ConditionalSplitTransformSearcher : ISSISItem
    {
        string currentItemType = "Conditional Split Transform";
        ...
        public List<PackageItem> Search(string searchPattern, object executable, PackageInfo packageData, string parentName = "", string parentType = "")
        {
            ...
            IDTSComponentMetaData100 conditionalSplitTransform = (IDTSComponentMetaData100)executable;

            currentItemName = (parentName != "") ? parentName + " -> " + conditionalSplitTransform.Name : conditionalSplitTransform.Name;
            currentItemType = (parentType != "") ? parentType + " -> " + currentItemType : currentItemType;

            if (CommonUtility.CheckStringExistence(conditionalSplitTransform.Name, searchPattern))
            {
                PackageItem taskNameItem = new PackageItem();
                taskNameItem.Type = currentItemType + " -> Name";
                taskNameItem.Name = currentItemName;
                taskNameItem.GUID = conditionalSplitTransform.ComponentClassID;
            ...
            if (CommonUtility.CheckStringExistence(conditionalSplitTransform.Description, searchPattern))
            {
                PackageItem taskDescItem = new PackageItem();
                taskDescItem.Type = currentItemType + " -> Description";
                taskDescItem.Name = currentItemName;
                taskDescItem.GUID = conditionalSplitTransform.ComponentClassID;
                taskDescItem.InterfaceName = packageData.Name;
                taskDescItem.FileName = packageData.FileName;
                SSISItems.Add(taskDescItem);
            }

            SSISItems.AddRange(SearchPattern(searchPattern, conditionalSplitTransform, packageData));
        ...
        private List<PackageItem> SearchPattern(string searchPattern, IDTSComponentMetaData100 component, PackageInfo packageData)
        {
            ...
            try
            {
                ...
                {
                    if (output.CustomPropertyCollection.Count > 1)
                    {
                        string columnName = output.Name;
                        ...
                        string columnPropertyValue = output.CustomPropertyCollection["FriendlyExpression"].Value.ToString();
