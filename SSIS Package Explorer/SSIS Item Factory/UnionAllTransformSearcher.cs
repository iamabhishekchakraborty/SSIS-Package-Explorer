...
{
    //Union All Transform
    public class UnionAllTransformSearcher:ISSISItem
    {
        string currentItemType = "Union All Transform";
        ...
        public List<PackageItem> Search(string searchPattern, object executable, PackageInfo packageData, string parentName = "", string parentType = "")
        {
            ...
            IDTSComponentMetaData100 unionAllTransform = (IDTSComponentMetaData100)executable;

            currentItemName = (parentName != "") ? parentName + " -> " + unionAllTransform.Name : unionAllTransform.Name;
            currentItemType = (parentType != "") ? parentType + " -> " + currentItemType : currentItemType;

            if (CommonUtility.CheckStringExistence(unionAllTransform.Name, searchPattern))
            {
                PackageItem taskNameItem = new PackageItem();
                taskNameItem.Type = currentItemType + " -> Name";
                taskNameItem.Name = currentItemName;
                taskNameItem.GUID = unionAllTransform.ComponentClassID;
