...
namespace SSIS_Package_Reader.SSIS_Item_Factory
{
    ...
    public class FuzzyGroupingTransformSearcher:ISSISItem
    {
        string currentItemType = "Fuzzy Grouping Transform";
        ...
        public List<PackageItem> Search(string searchPattern, object executable, PackageInfo packageData, string parentName = "", string parentType = "")
        {
            ...
            IDTSComponentMetaData100 fuzzyGroupingTransform = (IDTSComponentMetaData100)executable;

            currentItemName = (parentName != "") ? parentName + " -> " + fuzzyGroupingTransform.Name : fuzzyGroupingTransform.Name;
            currentItemType = (parentType != "") ? parentType + " -> " + currentItemType : currentItemType;

            if (CommonUtility.CheckStringExistence(fuzzyGroupingTransform.Name, searchPattern))
            {
                PackageItem taskNameItem = new PackageItem();
                taskNameItem.Type = currentItemType + " -> Name";
                taskNameItem.Name = currentItemName;
                taskNameItem.GUID = fuzzyGroupingTransform.ComponentClassID;
        ...
        private List<PackageItem> SearchPattern(string searchPattern, IDTSComponentMetaData100 component, PackageInfo packageData)
        {
            ...
            try
            {
                string expression = component.CustomPropertyCollection["Delimiters"].Value.ToString();
