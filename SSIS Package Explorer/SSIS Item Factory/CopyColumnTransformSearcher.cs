...
namespace SSIS_Package_Reader.SSIS_Item_Factory
{
    ...
    public class CopyColumnTransformSearcher:ISSISItem
    {
        string currentItemType = "Copy Column Transform";
        ...
        public List<PackageItem> Search(string searchPattern, object executable, PackageInfo packageData, string parentName = "", string parentType = "")
        {
            ...
            IDTSComponentMetaData100 copyColumnTransform = (IDTSComponentMetaData100)executable;

            currentItemName = (parentName != "") ? parentName + " -> " + copyColumnTransform.Name : copyColumnTransform.Name;
            currentItemType = (parentType != "") ? parentType + " -> " + currentItemType : currentItemType;

            if (CommonUtility.CheckStringExistence(copyColumnTransform.Name, searchPattern))
            {
                PackageItem taskNameItem = new PackageItem();
                taskNameItem.Type = currentItemType + " -> Name";
                taskNameItem.Name = currentItemName;
                taskNameItem.GUID = copyColumnTransform.ComponentClassID;
