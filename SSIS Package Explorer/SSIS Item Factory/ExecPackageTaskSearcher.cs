...
namespace SSIS_Package_Reader.SSIS_Item_Factory
{
    ...
    public class ExecPackageTaskSearcher : ISSISItem
    {
        string currentItemType = "Execute Package Task";
        ...
        public List<PackageItem> Search(string searchPattern, object executable, PackageInfo packageData, string parentName = "", string parentType = "")
        {
            ...
            ExecutePackageTask ExecutePackageTask = (ExecutePackageTask)hostedTask.InnerObject;
            ...
            SSISItems.AddRange(SearchPattern(searchPattern, ExecutePackageTask, hostedTask, packageData));

            return SSISItems;
        }

        private List<PackageItem> SearchPattern(string searchPattern, ExecutePackageTask task, TaskHost hostedTask, PackageInfo packageData)
        {
            List<PackageItem> SSISItems = new List<PackageItem>();

            if (CommonUtility.CheckStringExistence(task.PackageName, searchPattern))
            {
                PackageItem executeSQLTaskItem = new PackageItem();
                executeSQLTaskItem.Type = currentItemType + " -> PackageName";
